...
playerMovement({playerMoved,PlayerPid,PlayerRadius,PlayerX,PlayerY}, {MyRadius,MyX,MyY}) when sqrt(pow(PlayerX-MyX,2)+pow(PlayerY-MyY,2))>PlayerRadius-MyRadius -> %%when player doesn't cover dot
	{next_state, playerMovement, {MyRadius,MyX,MyY}}
;
playerMovement({playerMoved,PlayerPid,PlayerRadius,PlayerX,PlayerY}, {MyRadius,MyX,MyY})-> %when player is eaten
