...
init([]) ->
  init_db(),
  switch:start(),
  FoodServer = food_server:start(5),
...
handle_call({movePlayer,PlayerID,Dx,Dy}, _From, Tab) ->
  [{P,_X,_Y,_R,_ID,_Name}] = getPlayerById(PlayerID),
...
handle_call({register,PlayerID,Name}, _From, Tab) ->
  ...
  {ok,PID} = player:start({X,Y,10,PlayerID,Name}),
  insert_player({PID,X,Y,10,PlayerID,Name}),
  {reply,ok, Tab};
